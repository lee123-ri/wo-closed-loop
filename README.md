...
当前代码版本：**v0.7.0**。本地验证记录见 [测试报告](docs/测试报告.html)；生产部署与真实钉钉 OA 联调需另行验收。
...
> **用户与组织中心**（v0.7.0）：权限角色（菜单、操作、数据范围）与业务岗位分离。钉钉部门/项目群只生成岗位候选，管理员确认后才生效；通知规则只允许机器人私聊、机器人群聊，默认关闭且可先试算。运行事件与审计日志共同用于排障追溯。详见 [角色权限-通知运维方案](docs/角色权限-通知运维-方案.md)。
